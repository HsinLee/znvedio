      ...
      for (var path in spec.paths) {
        for (var method in spec.paths[path]) {
          ...
          for (var code in api.responses) {
            ...
            if (isOAS3) {
              // 注释掉源代码 update by boomer 2021-02-22 13:55:41
              // schema = response.content && response.content['application/json'] &&utils.inferSchema(response.content['application/json'])
              // 调整当是openapi 3.0版本时 兼容处理grpc+json update by boomer 2021-02-22 13:55:58
              var isOAS3JsonData;
              if (response.content) {
                if (response.content['application/grpc+json']) {
                  isOAS3JsonData = response.content['application/grpc+json']
                } else if (response.content['application/json']){
                  isOAS3JsonData = response.content['application/json']
                }
              }
              if (isOAS3JsonData) {
                schema = utils.inferSchema(isOAS3JsonData)
              }
